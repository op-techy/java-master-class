...
2. Create a package with your name and move `com.ope.Main.java` inside it:

```
src/com/<yourname>/com.ope.Main.java
```

For example, if your name is Franco: `src/com/franco/Main.java`

The starter `com.ope.Main.java` contains TODOs to guide the first steps:
...
graph TB
    subgraph Presentation
        M[com.ope.Main.java<br>CLI Menu Loop]
    end
...
- Implement the CLI menu loop in `com.ope.Main.java`
...
**Wiring in com.ope.Main.java** — the student can now swap between array and file implementations for bookings:
...
These can be swapped in via dependency injection in `com.ope.Main.java`:
...
│   │   └── com/
│   │       └── <yourname>/
│   │           ├── com.ope.Main.java                       (@SpringBootApplication)
│   │           ├── car/
│   │           │   ├── Brand.java
│   │           │   ├── Car.java                    (@Entity)
